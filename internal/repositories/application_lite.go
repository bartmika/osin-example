...
type ApplicationLiteRepo struct {
	db *sql.DB
}

func NewApplicationLiteRepo(db *sql.DB) *ApplicationLiteRepo {
	return &ApplicationLiteRepo{
		db: db,
	}
}

func (s *ApplicationLiteRepo) queryRowsWithFilter(ctx context.Context, query string, f *models.ApplicationLiteFilter) (*sql.Rows, error) {
	...
	// log.Println("ApplicationLiteRepo | query:", query, "\n")
	// log.Println("ApplicationLiteRepo | filterValues:", filterValues, "\n")
	// log.Println("ApplicationLiteRepo | f:", f, "\n")

	return s.db.QueryContext(ctx, query, filterValues...)
}

func (s *ApplicationLiteRepo) ListByFilter(ctx context.Context, filter *models.ApplicationLiteFilter) ([]*models.ApplicationLite, error) {
	...
	var arr []*models.ApplicationLite
	defer rows.Close()
	for rows.Next() {
		m := new(models.ApplicationLite)
		err := rows.Scan(
			&m.ID,
			&m.Name,
			&m.Scope,
	...
	if arr == nil {
		return []*models.ApplicationLite{}, nil
	}
	return arr, err
}

func (s *ApplicationLiteRepo) CountByFilter(ctx context.Context, f *models.ApplicationLiteFilter) (uint64, error) {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
